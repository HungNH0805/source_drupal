...
function minhhienstore2_form_webform_client_form_2423_alter(&$form, &$form_state, $form_id) {
  $form['actions']['submit']['#attributes']['class'] = array('btn btn-full btn-primary');
...
function sidebar_right_chuyen_muc()
{
  ...
  foreach ($mainMenu as $item){
    if($item['link']['hidden'] == 0){
      ...
      if(trim($item['link']['link_title'])=="Thùng tử dưới" || trim($item['link']['link_title'])=="Thùng tủ trên")
      {
        $str.='<div class="has-sub-menu-2"><p><i class="fa fa-angle-right" aria-hidden="true"></i> Kích thước tiêu chuẩn</p>';
        $grid = '3';
        $icon='<i class="fa fa-angle-double-right" aria-hidden="true"></i>';
      }
      if(count($item['below']) > 0){
        $count_sub = count($item['below']);
        $str .='<div class="has-sub-menu-'.$grid.'">';
        foreach($item['below'] as $subItem){
          if($subItem['link']['hidden'] == 0){
            if (count($subItem['below'])==0){

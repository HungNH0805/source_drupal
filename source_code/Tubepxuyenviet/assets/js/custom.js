...
$(document).on('change', '#edit-customer-profile-billing-field-thanh-pho-und', function(){
    ...
    $('.gia-phu').on('click',function(e){
        e.preventDefault();
        var variable = $(location).attr('href').split('/');
        var path_str='';
      ...
      $.ajax({
        url: 'https://bepxuyenviet.minhhien.com.vn/load-info-product',
        data: {valueGiaPhu: valueGiaPhu, productID: productID},
        dataType: 'json',
        type:'post',
        success: (data) => {
          $(this).parentsUntil('.product-form-cart').parent().parent().prev().prev().find('.info-product').html(data.mo_ta);
          $(this).parentsUntil('.product-form-cart').parent().parent().prev().find('.img-product').attr('srcset',data.img);
        }
      })
        ...
        $(this).parent().next().find(".add-to-cart-form").attr("href", '/commerce/add-to-cart/'+productID+'/'+quantity+'?path='+path_str);
  ...
  let heightImage;
  if (windowsize > 600){
    var heightSideRight = $('.grid-products-img').next().next().next().outerHeight() + $('.grid-products-img').next().next().next().find('img').outerHeight() + 30;
    var heightSideLeft = $('.grid-products-img').prev().outerHeight() + $('.grid-products-img').prev().find('img').outerHeight() + 30;
    if (heightSideRight < heightSideLeft){
      heightImage = heightSideLeft;
    }else {
      heightImage = heightSideRight;
    }
    var styles = {
      height : heightImage,
      'object-fit': "cover",
    }
    $('.grid-products-img').find('img').css(styles);
  }else{
      let heightImage = $(".grid-products-img").prev().find("img").height() - 6;
      var styles = {
        height: heightImage,
        "object-fit": "cover",
      };
      const img = $(".grid-products-img").find("img");
      img.css(styles);
  }
  $(".messages.error:contains(Array and string offset access syntax with curly braces is deprecated)").css("display","none");
  $(".messages.error ul li:contains(Array and string offset access syntax with curly braces is deprecated)").css("display","none");
})

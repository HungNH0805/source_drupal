jQuery(function () {
  ...
  $(document).ready(function () {
    ...
    const avatar = $('#edit-field-image');
    if (avatar.length > 0) {
      setInterval(function () {
        $('#edit-field-image-und-0-remove-button').val('X');
        let valueTitle = $('#san-pham-node-form #edit-title').val();
        $('#edit-field-image input[name^="field_image[und][0][alt]"]').val(valueTitle);
        $('#edit-field-image input[name^="field_image[und][0][title]"]').val(valueTitle);
      }, 1000)
    }
    if($('#admin-menu').length > 0){
      ...
      $('#san_pham_vip').removeClass('d-none');
    }
    if($('#edit-operation').length > 0){
      $('#edit-operation').addClass('form-control');
      $('#edit-operation').change(function(){
        $('#edit-submit--2').removeAttr('disabled');
        $('#edit-submit--2').addClass('btn btn-default');
      })
    }
    if($('#admin-menu-wrapper').length > 0){
      $(".form-item.form-type-checkboxes.form-item-roles").css('display','block');
      $(".form-item.form-type-textfield.form-item-name").css(
        "display",
        "block"
      );

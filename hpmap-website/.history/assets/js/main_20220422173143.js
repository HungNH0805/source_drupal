$(document).ready(function () {
  ...
  if ($(window).width() < 992) {
    $("#btn-loc-bds").click(function () {
      if ($(".chuyen_vi_tri #TimKiemSPPC").hasClass("ktra_form")) {
        ...
        $(".noi_hien_thi_form .btn-submit").html(
 ...
 --------------------------------------------------*/
  ...
  $(document).on("click", ".nice-select.gia_thue_can_ho ul li", function () {
    ...
    if (value !== "1") {
      location.assign(
        "/tat-ca-can-ho?sort_by=field_gia_value&sort_order=" + value
      );
    }
  });
  ...
  $(window).on("load", function () {
          ...
          success: function (data) {
            ...
            setTimeout(function (e) {
              ...
              if ($(window).width() > 1440) {
                $(".modal-dialog.modal-xl").css(
                  "margin-top",
                  ($("#modal-thong-tin-san-pham").height() -
                    $(".modal-dialog.modal-xl").height()) /
                    2 -
                    30
                );
                if (
                  $("#modal-thong-tin-san-pham #carouselExampleIndicators")
                    .length > 0
                ) {
                  $("#modal-thong-tin-san-pham .content").height(
                    $(
                      "#modal-thong-tin-san-pham #carouselExampleIndicators"
                    ).height()
                  );
                  $("#modal-thong-tin-san-pham .content").css(
                    "overflow-y",
                    "auto"
                  );
                }
    ...
    $(document).on(
      "click",
      ".leaflet-marker-icon.vip-div-icon.leaflet-zoom-animated.leaflet-interactive",
      function (e) {
        e.preventDefault();
        console.log(1);
        var $span = $(this).find("span.xem-chi-tiet-san-pham");
          ...
          success: function (data) {
            ...
            setTimeout(function (e) {
              ...
              if ($(window).width() > 1440) {
                $(".modal-dialog.modal-xl").css(
                  "margin-top",
                  ($("body").height() - $(".modal-dialog.modal-xl").height()) /
                    2 -
                    30
                );
                if (
    ...
    $("#edit-field-so-dien-thoai-vn-und-0-value").on(
      "keypress keyup blur",
      function (event) {
        $(this).val(
          $(this)
          ...
          if (
            ...
            if (r1[0].status === 0) {
              if ($("#error-phone").length > 0) {
                ...
                $(
                  "#user-register-form #edit-field-so-dien-thoai-vn-und-0-value"
                ).after(
                  '<p class="text-danger mt-10" id="error-phone"> * Số điện thoại đã được đăng kí. </p>'
                );
              }
              $(
                "#user-register-form #edit-field-so-dien-thoai-vn-und-0-value"
              ).css("border", "1px solid #fe0202");
            } else {
              $(
                "#user-register-form #edit-field-so-dien-thoai-vn-und-0-value"
              ).css("border", "1px solid #027bfe");
              $("#error-phone").remove();
            }
            if (r2[0].status === 0 || !value.match(illegalChars)) {
              if (r2[0].status === 0) {
                if ($("#error-username").length > 0) {
                  $("#error-username").text(
                    "* Tên người dùng đã được đăng kí."
                  );
                } else {
                  $("#user-register-form #edit-name").after(
                    '<p class="text-danger mt-10" id="error-username"> * Tên người dùng đã được đăng kí. </p>'
                  );
                }
              } else if (!value.match(illegalChars)) {
                if ($("#error-username").length > 0) {
                  $("#error-username").text(
                    "* Tên người dùng không gồm dấu và chữ viết hoa."
                  );
                } else {
                  $("#user-register-form #edit-name").after(
                    '<p class="text-danger mt-10" id="error-username"> * Tên người dùng không gồm dấu và chữ viết hoa. </p>'
                  );
                }
              }
              $("#user-register-form #edit-name").css(
                "border",
                "1px solid #fe0202"
              );
            } else {
            ...
            if (pass1 !== pass2) {
              if ($("#edit-pass2").length > 0) {
                ...
                $("#edit-pass-pass2").after(
                  '<p class="text-danger mt-10" id="edit-pass2"> * Mật khẩu không trùng nhau. </p>'
                );
              }
    ...
    if ($("#hidden-nid-node").length > 0 || $("#block-block-12").length > 0) {
      $.ajax({
        url: "https://app.hpmap.vn/service/get-info-relation",
        data: {
          nid: $("#hidden-nid-node").val(),
        },
        dataType: "json",
        type: "post",
        before: function () {},
        success: function (data) {
          ...
          <div class="thumb">
            <a href="#" class="view-more-info-user" data-value="` +
              data.saler.uid +
              `">
              <img src="` +
              data.saler.anh_nguoi_dung +
              `" alt="` +
              data.saler.hoten +
              `"  class="img-fluid">
            </a>
          ...
          <div class="details">
            ...
            <span class="designation">
              ...
              <a class="btn-primary btn bg-white color-red" id="btn-call-now" href="#" data-uid="` +
              data.saler.uid +
              `"> <i class="fas fa-phone"></i> <span href="#" class="phone_number_saler">` +
              data.saler.dien_thoai +
              `</span></a>
            </span>
            <div class="hidden"><a class="btn-contact btn-primary" id="btn-call-me-now" href="#"></a></div>
          </div>
        </div>`
          );
          ...
          if ($("#block-block-12").length > 0) {
            require([
              ...
              esriConfig.apiKey =
                "AAPK3e5990abf0eb4ddc85ce5776943cbcc7KrLptUnOIT1zRnHXg9MngxirG_N_BSk-iEY__lvYqfRWEZDlPkHx8Ked9aZsZ9Ud";
              ...
              const view = new MapView({
                map: map,
                center: [106.691932, 20.81919], // Longitude, latitude
                zoom: 13, // Zoom level
                container: "block-block-12", // Div element
              });
    ...
    $(document).on("click", "#btn-call-now", function (e) {
      ...
      var $me = $(this);
      $.ajax({
        url: "https://app.hpmap.vn/service/get-full-phone-number",
        dataTpe: "json",
        type: "post",
        data: {
        ...
        success: function (data) {
          $me.parent().find(".phone_number_saler").html(data.phone_number);
          $("#btn-call-me-now").attr("href", "tel:" + data.phone_number);
  ...
  if ($("#san-pham-node-form #edit-field-quan-huyen-und-0-value").length > 0) {
    $("#san-pham-node-form #edit-field-quan-huyen-und-0-value").prepend(
      "<p>Hiện Hpmap đang triển khai tại thị trường Hải Phòng Vui lòng chọn quận huyện</p>"
    );
  }
});

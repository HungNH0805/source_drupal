...
function getMenu2LeMinhLand(){
      ...
      else
      {
        ...
        if (count($item['below']) > 0) {
          ...
          foreach ($item['below'] as $subItem) {
            ...
            if ($subItem['link']['hidden'] == 0)
              if(trim($subItem['link']['link_title'])=='Quản lý sản phẩm'||trim($subItem['link']['link_title'])=='Quản lý khách hàng')
              {
                $str .= "<li>" . l(
  ...
  if ($user->uid != 0){
    ...
    $temp = explode(" ", $user_detail->field_ho_ten['und'][0]['value']);
    $fullname = end($temp);
      $str.='<li class="menu-item-has-children current-menu-item"><i class="d-none-pc fas fa-caret-down"></i>
                <a href="/user"><i class="fas fa-user"></i> '. $fullname. '</a>
                <ul class="sub-menu-1">
                <li><a href="/user/'.$user->uid.'"><i class="fas fa-tablet-alt"></i> Thông tin cá nhân</a></li>
                <li><a href="/user/logout" class="ml-10"><i class="fas fa-sign-out-alt"></i> Đăng xuất</a></li>
                </ul>
            </li>';
  }
...
function leminhland_form_views_exposed_form_alter(&$form, &$form_state, $form_id) {
  ...
  if($form['#id'] == 'views-exposed-form-admin-views-node-page-quan-tri-vien'){
    $form['title']['#attributes']['class'] = array('form-control');
    $form['type']['#attributes']['class'] = array('form-control');
    $form['status']['#attributes']['class'] = array('form-control');
    $form['vid']['#attributes']['class'] = array('form-control');
    $form['nid']['#attributes']['class'] = array('form-control');
    $form['submit']['#prefix'] = t('<div class="btn-content mr-30">');
    $form['reset']['#suffix'] = t('</div>');
    $form['submit']['#attributes']['class'] = array('btn btn-primary');
    $form['reset']['#attributes']['class'] = array('btn btn-default');
  }
}
...
{
  $form['#prefix']=t('<div class="row justify-content-center"><div class="col-md-4">');
  $form['#suffix']=t('</div></div>');
  $form['name']['#attributes']['class'] = array('form-control');
  $form['pass']['#attributes']['class'] = array('form-control');
  $form['actions']['#theme_wrappers'] = array();
  $form['actions']['submit']['#attributes']['class'] = array('btn btn-primary mr-10');
  $form['actions']['#prefix']='<div class="d-flex">';
  $form['actions']['#suffix']='<a href="/user/register" class="btn btn-primary">Đăng ký</a></div>';
}
 ...
 * Implements hook_form_alter().
 */
function leminhland_form_user_profile_form_alter(&$form, &$form_state, $form_id) {
  $form['account']['name']['#attributes']['class'] = array('form-control','border_xanh');
  $form['field_ho_ten']['und'][0]['value']['#attributes']['class'] = array('form-control');
  $form['field_ho_ten']['#prefix'] = t('<div class="row"><div class="col-md-3">');
  $form['field_ho_ten']['#suffix'] = t('</div>');
  $form['account']['#prefix'] = t('<div class="col-md-9">');
  $form['account']['#suffix'] = t('</div></div>');
  $form['account']['current_pass']['#prefix'] = t('<div class="row"><div class="col-md-4">');
  $form['account']['current_pass']['#suffix'] = t('</div>');
  $form['account']['pass']['#prefix'] = t('<div class="col-md-8">');
  $form['account']['pass']['#suffix'] = t('</div></div>');
  $form['field_so_dien_thoai_vn']['#prefix'] = t('<div class="row"><div class="col-md-3">');
  $form['field_so_dien_thoai_vn']['#suffix'] = t('</div>');
  $form['field_dia_chi_du_an']['#prefix'] = t('<div class="col-md-9">');
  $form['field_dia_chi_du_an']['#suffix'] = t('</div></div>');
  $form['field_dt_nguoi_gioi_thieu']['#prefix'] = t('<div class="row"><div class="col-md-3">');
  $form['field_dt_nguoi_gioi_thieu']['#suffix'] = t('</div><div class="col-md-9"><div class="input-image"></div></div></div>');
  $form['timezone']['#prefix'] = t('<div class="d-none">');
  $form['timezone']['#suffix'] = t('</div>');
}

/**
 * Implements hook_form_FORM_ID_alter().
 */
function leminhland_form_alter (&$form, &$form_state, $form_id) {
  if($form['#id'] == "views-exposed-form-admin-views-user-system-1")
  {
//    $form['rid']['#theme_wrappers'] = array();
//    $form['name']['#theme_wrappers'] = array();
//    $form['mail']['#theme_wrappers'] = array();
//    $form['status']['#theme_wrappers'] = array();
//    $form['rid']['#prefix']='<div class="row"><div class="col-md-3">';
//    $form['rid']['#suffix']='</div>';
//    $form['name']['#prefix']='<div class="col-md-3">';
//    $form['name']['#suffix']='</div>';
//    $form['mail']['#prefix']='<div class="col-md-3">';
//    $form['mail']['#suffix']='</div>';
//    $form['status']['#prefix']='<div class="col-md-3">';
//    $form['status']['#suffix']='</div></div>';
    $form['submit']['#attributes']['class']=array('btn btn-primary');
    $form['reset']['#attributes']['class']=array('btn btn-primary');
    $form['rid']['#attributes']['class']=array('form-control');
    $form['name']['#attributes']['class']=array('form-control');
    $form['mail']['#attributes']['class']=array('form-control');
    $form['status']['#attributes']['class']=array('form-control');
//    dsm($form);
  }
  if($form['#id'] == "user-admin-roles"){
    $form['add']['#attributes']['class'] = array('btn btn-primary');
    $form['actions']['submit']['#attributes']['class'] = array('btn btn-primary');
    $form['name']['#attributes']['class'] = array('form-control','mt-0');
//    dsm($form);
  }
}
